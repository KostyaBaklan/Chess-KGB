...
{
    public class LateMoveReductionDeepLinearStrategy : LateMoveReductionStrategyBase
    {
        public LateMoveReductionDeepLinearStrategy(short depth, IPosition position, TranspositionTable table = null)
        ...
        {
            return new LateMoveReductionDeepLinearStrategy((short)(Depth - LmrSubSearchDepth), Position);
        }
            ...
            for (int depth = 0; depth < result.Length; depth++)
            {
                ...
                for (int move = 0; move < result[depth].Length; move++)
                {
                    if (depth > 6)
                    {
                        if (move > 24)
                        {
                            result[depth][move] = (byte)(depth - 4);
                        }
                        else if (move > 11)
                        {
